...
!function () {
  ...
  function a (a, b, c) {'addEventListener' in window ? a.addEventListener(b, c, !1) : 'attachEvent' in window && a.attachEvent('on' + b, c)}
  ...
  function g (a) {
    function b () {
      function a () {k(F), i(), C.resizedCallback(F)}
    ...
    function g (a) {
      var b = Number(C['max' + a]), c = Number(C['min' + a]), d = a.toLowerCase(), f = Number(F[d])
    ...
    function m () {
      var b = a.origin, c = F.iframe.src.split('/').slice(0, 3).join('/')
      if (C.checkOrigin && (e(' Checking connection is from: ' + c), '' + b != 'null' && b !== c)) throw new Error('Unexpected message received from: ' + b + ' for ' + F.iframe.id + '. Message was: ' + a.data + '. This error can be disabled by adding the checkOrigin: false option.')
      return !0
    }
    ...
    function q (a) {
      e(' MessageCallback passed: {iframe: ' + F.iframe.id + ', message: ' + a + '}'), C.messageCallback({
        iframe: F.iframe,
    ...
    function u (a) {
      ...
      var d = a ? s(F.iframe) : { x: 0, y: 0 }, g = c()
      e(' Reposition requested from iFrame (offset x:' + d.x + ' y:' + d.y + ')'), window.top !== window.self ? window.parentIFrame ? a ? parentIFrame.scrollToOffset(g.x, g.y) : parentIFrame.scrollTo(F.width, F.height) : f(' Unable to scroll to requested position, window.parentIFrame not found') : b()
    }
    ...
    function B (a) {
      ...
      var c = a.split('#')[1] || '', d = decodeURIComponent(c),
  ...
  function j (a) {
    function b () {k(a), m('reset', 'reset', a.iframe)}

    e(' Size reset requested by ' + ('init' === a.type ? 'host page' : 'iFrame')), h(), l(b, a, 'init')
  }

  function k (a) {
    function b (b) {a.iframe.style[b] = a[b] + 'px', e(' IFrame (' + a.iframe.id + ') ' + b + ' set to ' + a[b] + 'px')}
  ...
  function n () {
    function b () {
      function a (a) {1 / 0 !== C[a] && 0 !== C[a] && (i.style[a] = C[a] + 'px', e(' Set ' + a + ' = ' + C[a] + 'px'))}
    ...
    function g () {return k + ':' + C.bodyMarginV1 + ':' + C.sizeWidth + ':' + C.log + ':' + C.interval + ':' + C.enablePublicMethods + ':' + C.autoResize + ':' + C.bodyMargin + ':' + C.heightCalculationMethod + ':' + C.bodyBackground + ':' + C.bodyPadding + ':' + C.tolerance + ':' + C.enableInPageLinks}

    function h (b) {
      a(i, 'load', function () {
        var a = t
        m('iFrame.onload', b, i), !a && C.heightCalculationMethod in B && j({
          iframe: i,
          height: 0,
          width: 0,
          type: 'init'
  ...
  var s = 0, t = !0, u = 'message', v = u.length, w = '[iFrameSizer]', x = w.length, y = '', z = null,
    A = window.requestAnimationFrame, B = { max: 1, scroll: 1, bodyScroll: 1, documentElementScroll: 1 }, C = {}, D = {
      autoResize: !0,
      bodyBackground: null,
      bodyMargin: null,
      bodyMarginV1: 8,
  ...
  b(), a(window, 'message', g), window.jQuery && r(jQuery), 'function' == typeof define && define.amd ? define([], q) : 'object' == typeof module && 'object' == typeof module.exports ? module.exports = q() : window.iFrameResize = q()
}()
